...
/**
 * Base class that represents a row from the 'gameplayers' table.
 *
 *
 *
 * @package    propel.generator.Model.Base
 */
abstract class Gameplayers implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\Map\\GameplayersTableMap';
    ...
    /**
     * Initializes internal state of Model\Base\Gameplayers object.
     */
    ...
    /**
     * Compares this with another <code>Gameplayers</code> instance.  If
     * <code>obj</code> is an instance of <code>Gameplayers</code>, delegates to
     * <code>equals(Gameplayers)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     * @return $this|\Model\Gameplayers The current object (for fluent API support)
     */
    public function setPlayerid($v)
    {
        ...
        if ($this->playerid !== $v) {
            $this->playerid = $v;
            $this->modifiedColumns[GameplayersTableMap::COL_PLAYERID] = true;
        }
    ...
    public function setGameid($v)
    {
        ...
        if ($this->gameid !== $v) {
            $this->gameid = $v;
            $this->modifiedColumns[GameplayersTableMap::COL_GAMEID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : GameplayersTableMap::translateFieldName('Playerid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->playerid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GameplayersTableMap::translateFieldName('Gameid', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = GameplayersTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Model\\Gameplayers'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildGameplayersQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Gameplayers::setDeleted()
     * @see Gameplayers::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildGameplayersQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                GameplayersTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(GameplayersTableMap::COL_PLAYERID)) {
            $modifiedColumns[':p' . $index++]  = 'playerid';
        }
        if ($this->isColumnModified(GameplayersTableMap::COL_GAMEID)) {
            $modifiedColumns[':p' . $index++]  = 'gameid';
        }

        $sql = sprintf(
            'INSERT INTO gameplayers (%s) VALUES (%s)',
    ...
    {
        $pos = GameplayersTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Gameplayers'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Gameplayers'][$this->hashCode()] = true;
        $keys = GameplayersTableMap::getFieldNames($keyType);
        $result = array(
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\Gameplayers The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GameplayersTableMap::COL_PLAYERID)) {
            $criteria->add(GameplayersTableMap::COL_PLAYERID, $this->playerid);
        }
        if ($this->isColumnModified(GameplayersTableMap::COL_GAMEID)) {
            $criteria->add(GameplayersTableMap::COL_GAMEID, $this->gameid);
        }
    ...
    {
        $criteria = ChildGameplayersQuery::create();
        $criteria->add(GameplayersTableMap::COL_PLAYERID, $this->playerid);
        $criteria->add(GameplayersTableMap::COL_GAMEID, $this->gameid);
    ...
    {
        return (null === $this->getPlayerid()) && (null === $this->getGameid());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Model\Gameplayers (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Model\Gameplayers Clone of current object.
     * @throws PropelException
     */
    ...
    public function getGame(ConnectionInterface $con = null)
    {
        if ($this->aGame === null && ($this->gameid != 0)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aGame->addGameplayerss($this);
             */
        }
    ...
    public function getPlayer(ConnectionInterface $con = null)
    {
        if ($this->aPlayer === null && ($this->playerid != 0)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPlayer->addGameplayerss($this);
             */
        }
    ...
    {
        if (null !== $this->aGame) {
            $this->aGame->removeGameplayers($this);
        }
        if (null !== $this->aPlayer) {
            $this->aPlayer->removeGameplayers($this);
        }
        $this->playerid = null;
        $this->gameid = null;
        $this->alreadyInSave = false;
    ...
    {
        return (string) $this->exportTo(GameplayersTableMap::DEFAULT_STRING_FORMAT);
    }
